...
| 👤 내 정보 | 회원 로그인 후 소속·에버·대회 이력(순위, 게임 점수, 클럽 순위, 포인트)·통계 확인, 연락처·PIN 변경 |
...
### 권한
- **관리자**: 관리자 PIN 로그인. 모든 데이터 편집 가능. (회원 계정에 관리자 역할을 부여할 수도 있음)
- **회원**: 클럽 + 이름 + PIN 로그인. 조회 + 본인 연락처/PIN 변경.
- **게스트**: 로그인 없이 대회 일정·순위·랭킹 조회.
...
5. 웹앱 로그인 화면의 **⚙ 서버 연결 설정** 또는 설정 탭 → 서버 연결에 URL 입력
   (모든 기기에서 기본으로 쓰려면 `js/store.js` 의 `DEFAULT_API_URL` 에 넣고 배포)

시트(`클럽`, `회원`, `대회`, `설정`)는 첫 호출 시 자동 생성됩니다. 회원 PIN은 서버에서만 확인하며 클라이언트로 전송되지 않습니다.
로그인 토큰은 6시간 동안 유효합니다.
...
member     { id, name, clubId, gender:'M'|'F', avg, role:'member'|'admin', phone, joinDate, note }  // pin은 서버 전용
