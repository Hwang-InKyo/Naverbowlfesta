...
| 지역 대표 5인조 | 지역별 5명, 이벤트 | 1 | 결과 (기본은 포인트 미반영) |

**지역 종합 포인트** = 개인전 남/여 상위 N명 순위 포인트 + 지역 대표(지역당 3명) 개인전 합계 순위 포인트 + 스카치 상위 팀 + 베이커 상위 팀. 합계가 가장 많은 지역이 우승기. 배점은 설정 탭에서 바꿀 수 있습니다.

| 탭 | 내용 |
|---|---|
| 🏠 홈 | 대회 정보, 일정, 종목별 입력 진행률, 지역 종합·개인전 현재 순위 |
| 👥 선수 | 참가 선수 명단(지역/성별/검색), 핸디, 조·레인, 출전 종목. 관리자: 지역 관리, 선수 등록/수정, 신청서 일괄 등록 |
| 🎯 배정 | 개인전 조별 레인표, 스카치/베이커/5인조 팀. 관리자: 조 자동 편성(지역별 균등), 레인 자동 배정(같은 지역 분산), 팀 만들기, 직접 수정 |
| 🎳 개인전 | 남/여/전체/조별 순위. 관리자: 조별 점수 입력(자동 저장) |
| 👫 스카치 · 🤝 베이커 · 🏁 5인조 | 팀 순위. 관리자: 점수 입력 |
| 🏆 지역종합 | 지역 종합 순위(포인트 상세), 지역 대표 합계 순위. 관리자: 대회 시작/최종 확정/확정 해제 |
| ⚙️ 설정 | 대회 정보, 조 구성, 핸디 규정, 순위 기준, 팀 핸디 방식, 게임 수, 배점표, 관리자 PIN, 서버 연결, 백업 |

### 규정 (설정에서 변경 가능)
- 핸디(게임당) = ⌊(기준 − 에버) × 비율⌋, 최대치, 여성 추가 핀. 선수별 수동 핸디 가능
- 개인전 순위 기준: 핸디 포함 총점(기본) 또는 스크래치. 동점은 총점 → 스크래치 → 하이게임 → 마지막 게임 순, 완전 동점은 공동 순위
- 팀 핸디: 팀원 평균(기본) / 합계 / 없음
...
```
이름,지역,성별(남/여),에버,종목,대표
홍길동,서울,남,185,개인 스카치 5인조,대표
김영희,서울,여,160,개인 베이커
```
...
settings { name, venue, dates[2], hostRegionId, status:'ready'|'live'|'final', lanes, laneFrom, perLane,
           groups:[{id,name,day,time}], handicap:{type,base,rate,cap,femaleBonus}, basis, teamHandicap, games,
           repCount, points:{individual,reps,scotch,baker,team5}, countTeam5, schedule }
region   { id, name, leader, note }
player   { id, name, regionId, gender, avg, handicapOverride, isRep, group, lane, pos, games[], events:{individual,scotch,baker,team5}, note }
team     { id, event:'scotch'|'baker'|'team5', regionId, name, members:[playerId], lane, games[] }
